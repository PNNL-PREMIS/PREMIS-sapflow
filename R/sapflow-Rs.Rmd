...
wx_dat %>% 
  gather(var, value, PAR, Precip, RH, Tair) %>% 
  mutate(Timestamp = with_tz(Timestamp, "EST"), 
         daytime = Timestamp >= sunrise & Timestamp <= sunset,
         month = month(Timestamp),
         hour = hour(Timestamp)) %>% 
...
fsd <- function(sunny_days, climate_data, constraints, lookahead) {
  matches <- list()
...
fsd(sunny_days, climate_data, constraints, lookahead = 7) %>% 
