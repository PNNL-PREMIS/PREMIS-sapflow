...
read_csv("../SERC/met_data/PARPAR_30min.csv") %>% 
  mutate(Timestamp = ymd_hms(endDateTime), DOY = yday(Timestamp), Date = date(Timestamp)) %>% 
  rename(PAR = PARMean) %>% 
  filter(verticalPosition == "050") %>% 
  select(Timestamp, DOY, PAR, Date) %>%
  ...
  summarise(DOY = mean(DOY), PAR = mean(PAR), 
            Precip = mean(Precip), RH = mean(RH), Tair = mean(Tair), Date = mean(Date)) -> wx_dat

# Compute daytime PAR as a new column
# To split PAR groups, we need to use daily averages of daytime values
sunlight_times <- getSunlightTimes(wx_dat$Date, lat = 38.9, lon = -77, tz = "UTC", keep = c("sunrise", "sunset"))

wx_dat$sunrise <- sunlight_times$sunrise
wx_dat$sunset <- sunlight_times$sunset
# Check time zones before this step
wx_dat %>% 
  filter(Timestamp >= sunrise, Timestamp <= sunset) %>% 
  group_by(DOY) %>% 
  summarise(Daytime_PAR = mean(PAR, na.rm = TRUE)) %>% 
  right_join(wx_dat, by = "DOY")-> x
