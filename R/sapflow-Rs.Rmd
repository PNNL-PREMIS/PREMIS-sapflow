...
combined %>% 
  ungroup() %>% 
  complete(Tree, Timestamp = seq(ymd_hms(min(Timestamp)), ymd_hms(max(Timestamp)), by = "hour")) -> full
...
ggplot(data = q2, aes(x = as.numeric(Week), y = Hour, fill = Cor)) + 
  ...
  facet_wrap(~Tree) + 
  theme_minimal() + 
  scale_fill_distiller(palette = "RdBu") -> p1
...
# Note this splits the daily averages up
...
full_sc %>%
  group_by(Tree, Coverage) %>%
  do(compute_lag_cor(.$rs_avg, .$Js_avg, hour_range = c(0,23))) -> q3a

ggplot(data = q3a, aes(x = Hour, y = Cor, color = Coverage)) + 
  geom_line() +
  facet_wrap(~Tree) +
  theme_minimal()
...
# Lag cor analysis of matched days
matches_data %>% 
