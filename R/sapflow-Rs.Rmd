...
# Load function to find matching days
...
```{r sapwood-area, echo = FALSE}

inventory %>% 
  filter(Plot == "Control", Species_code != "FAGR") %>% 
  drop_na(Sapflux) %>% 
...
sapwood_depth <- read_csv("~/Desktop/sapwood_depthsSERC.csv", 
                          col_names = c("DBH", "sapwood_d", "Species"), 
                          skip = 1) # read in randomly sampled sapwood depths at site
...
js_trees %>% 
  ...
  mutate(t_area_cm2 = pi * (DBH / 2)^2, hw_area_cm2 = pi * ((DBH - 2 * pred_sd) / 2) ^ 2) %>% 
  mutate(sw_area_cm2 = t_area_cm2 - hw_area_cm2) -> sapwood_area
...
sapflow_k_long %>% 
  ...
  summarise(Js_avg = mean(Js, na.rm = TRUE),
            .groups = "drop") %>% 
  log_obs("summarise sapflow_js by hour") -> sapflow_js
...
combined %>% 
  group_by(Timestamp) %>% 
  summarise(Rs = mean(rs_avg, na.rm = TRUE), Js = mean(Js_avg, na.rm = TRUE)) %>% 
  do(compute_lag_cor(.$Rs, .$Js, hour_range = c(0, 23))) -> q1_eco 
...
growing %>% 
  ...
  summarise(Rs = mean(rs_avg, na.rm = TRUE), 
            Js = mean(Js_avg, na.rm = TRUE), Week = mean(Week)) %>% 
  group_by(Week) %>% 
  do(compute_lag_cor(.$Rs, .$Js, hour_range = c(0, 23))) %>% 
  ungroup() %>% 
  na.omit() -> q2_eco 
...
ggplot(matches_daily, aes(Species_code, Daily_Js_avg, color = Coverage)) + 
  geom_boxplot() 
