...
read_csv("../SERC/met_data/SECPRE_30min.csv") %>% 
  mutate(Timestamp = ymd_hms(endDateTime, tz = "EST")) %>% 
...
read_csv("../SERC/met_data/PARPAR_30min.csv") %>% 
  ...
  mutate(DOY = yday(Timestamp), 
         Date = date(Timestamp)) -> wx_dat

# Compute daytime PAR as a new column
sunlight_times <- getSunlightTimes(wx_dat$Date, lat = 38.9, lon = -77, tz = "EST", keep = c("sunrise", "sunset"))
...
wx_dat %>% 
  ...
  right_join(wx_dat, by = "DOY")-> wx_dat
```
...
ggplot(wx_dat, aes(DOY, PAR)) + 
  ...
  geom_point(aes(y = Daytime_PAR), color = "red")
```
