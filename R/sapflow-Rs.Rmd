...
compute_lag_cor <- function(rs, js, length) {
  # `rs` and `js` are numeric vectors with the same length
  # length is a numeric vector of length 2 that contains the start and end hour
  
  for(i in length[1]:length[2]) {
#    if (i %in% dataset$Hour) stop("Dataset not complete: missing hour")
   
    js_lag <- lead(js, n = i)
    cor <- cor(rs, js_lag, use = "na.or.complete")
    cor_hour[[i+1]] <<- tibble(Hour = i, Cor = cor)
...
## Science Questions. {.tabset .tabset-pills}
...
group_map(grouped, compute_lag_cor(rs_avg, Js_avg, length = c(0,23)))
...
```{r sunny-cloudy, echo = FALSE, message = FALSE, warning = FALSE, include = FALSE}
...
```{r lag-analysis-matches, echo = FALSE, message = FALSE}
